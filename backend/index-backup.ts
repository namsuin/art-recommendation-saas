...
const server = Bun.serve({
  ...
  async fetch(req: Request) {
    ...
    if (pathname === "/api/auth/upload-limit" && req.method === "GET") {
        try {
          const url = new URL(req.url);
    ...
    // 고급 개인화 추천 API (V2)
    if (pathname === "/api/v2/recommendations" && req.method === "POST") {
        try {
    ...
    if (pathname === "/api/v2/interactions" && req.method === "POST") {
        try {
    ...
    if (pathname === "/api/v2/recommendations/realtime" && req.method === "GET") {
        try {
          const url = new URL(req.url);
          const userId = url.searchParams.get('userId');
    ...
    if (pathname === "/api/v2/analytics/recommendations" && req.method === "GET") {
        try {
          const url = new URL(req.url);
          const userId = url.searchParams.get('userId');
    ...
    // 소셜 기능 API
    if (pathname === "/api/social/share" && req.method === "POST") {
        try {
    ...
    if (pathname === "/api/social/review" && req.method === "POST") {
        try {
    ...
    if (pathname === "/api/social/follow" && req.method === "POST") {
        try {
    ...
    if (pathname === "/api/social/interact" && req.method === "POST") {
        try {
    ...
    if (pathname === "/api/social/comment" && req.method === "POST") {
        try {
    ...
    if (pathname === "/api/social/trending" && req.method === "GET") {
        try {
          const url = new URL(req.url);
    ...
    // AI 성능 모니터링 API
    if (pathname === "/api/ai/performance" && req.method === "GET") {
        try {
    ...
    if (pathname === "/api/ai/optimization/config" && req.method === "PUT") {
        try {
    ...
    if (pathname === "/api/ai/cache/clear" && req.method === "POST") {
        try {
    ...
    if (pathname === "/api/subscription/resume" && req.method === "POST") {
        try {
          const { userId, subscriptionId } = await req.json();
    ...
    if (pathname === "/api/webhook/stripe" && req.method === "POST") {
        try {
    ...
    // Admin API endpoints
    if (pathname === "/api/admin/stats/users" && req.method === "GET") {
        try {
          const url = new URL(req.url);
          const userId = url.searchParams.get('userId');
    ...
    if (pathname === "/api/admin/stats/usage" && req.method === "GET") {
        try {
          const url = new URL(req.url);
          const userId = url.searchParams.get('userId');
    ...
    if (pathname === "/api/admin/artworks" && req.method === "GET") {
        try {
          const url = new URL(req.url);
          const userId = url.searchParams.get('userId');
    ...
    if (pathname === "/api/admin/artworks" && req.method === "POST") {
        try {
    ...
    if (pathname.startsWith("/api/admin/artworks/") && req.method === "PUT") {
        try {
          const url = new URL(req.url);
    ...
    if (pathname.startsWith("/api/admin/artworks/") && req.method === "DELETE") {
        try {
          const url = new URL(req.url);
          const artworkId = url.pathname.split('/').pop();
          const userId = url.searchParams.get('userId');
    ...
    if (pathname === "/api/analytics/system" && req.method === "GET") {
        try {
          const url = new URL(req.url);
          const userId = url.searchParams.get('userId');
          const days = parseInt(url.searchParams.get('days') || '30');
    ...
    if (pathname === "/api/analytics/recommendations" && req.method === "GET") {
        try {
          const url = new URL(req.url);
          const userId = url.searchParams.get('userId');
    ...
    if (pathname === "/api/purchase/request" && req.method === "POST") {
        try {
    ...
    if (pathname === "/api/purchase/requests" && req.method === "GET") {
        try {
          const url = new URL(req.url);
          const userId = url.searchParams.get('userId');
    ...
    if (pathname === "/api/purchase/requests/:id/cancel" && req.method === "POST") {
        try {
          const url = new URL(req.url);
    ...
    if (pathname === "/api/admin/purchase/requests" && req.method === "GET") {
        try {
          const url = new URL(req.url);
          const userId = url.searchParams.get('userId');
          const page = parseInt(url.searchParams.get('page') || '1');
    ...
    if (pathname === "/api/admin/purchase/requests/:id/status" && req.method === "PUT") {
        try {
          const url = new URL(req.url);
    ...
    if (pathname === "/api/multi-image/analyze" && req.method === "POST") {
        return await getMultiImageAPI().analyzeMultipleImages(req);
    }
    
    if (pathname === "/api/multi-image/history" && req.method === "GET") {
        try {
          const url = new URL(req.url);
          const userId = url.searchParams.get('userId');
          const limit = parseInt(url.searchParams.get('limit') || '20');
    ...
    if (pathname === "/api/multi-image/payment-success" && req.method === "POST") {
        try {
    ...
    if (pathname === "/api/search/external" && req.method === "POST") {
        return await getMultiImageAPI().searchExternalPlatforms(req);
      }
    
    if (pathname === "/api/artsonia/search" && req.method === "GET") {
        try {
    ...
    if (pathname === "/api/artsonia/availability" && req.method === "GET") {
        try {
    ...
    if (pathname === "/api/admin/clean-tumblbug" && req.method === "POST") {
        try {
    ...
    if (pathname === "/api/admin/check-tumblbug" && req.method === "GET") {
        try {
          const { TumblbugDataCleaner } = await import('./utils/clean-tumblbug-data');
          const cleaner = new TumblbugDataCleaner();
